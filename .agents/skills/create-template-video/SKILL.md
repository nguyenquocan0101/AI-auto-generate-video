...
**HOOK (chỉ 1):** `frame-liquid-bg-hero` — LUÔN dùng cho scene hook.

**BODY (Claude tự chọn — ưu tiên ĐA DẠNG, không lặp 1 template):**
- `frame-vignelli` — 1 con số/stat nền tối charcoal + đỏ
- `frame-pentagram-stat` — 1 con số nền tối neon (cam + cyan)
- `frame-bold-poster` — tuyên bố mạnh nhiều dòng + figure số lớn
- `frame-build-minimal` — 1 từ chốt reveal từng chữ, nền tối + amber
- `frame-creative-voltage` — câu sáng tạo, split xanh điện + viết tay
- `frame-glitch-title` — tin sốc/breaking cyberpunk glitch
- `frame-aicoding-list` — danh sách 2–5 mục (icon + level tag)
- `frame-aicoding-comparison` — so sánh 2 thứ (A vs B)

**OUTRO (chỉ 1):** `frame-logo-outro` — mặc định. Hoặc `frame-statement-outro` (card đỏ).

---

### Step 5: Sinh script.json (8–12 scene)

- **Hook** → `frame-liquid-bg-hero` (slots: kicker, headline, subheadline, cta, brand)
- **Body** → Claude TỰ CHỌN template hợp nhất cho từng scene (xem CATALOG.md cho slot reference đầy đủ)
- **Outro** → `frame-logo-outro` (slots: brand_name, tagline, primary_url)
...
Gợi ý cân bằng:
- Scene số liệu: xen kẽ `frame-vignelli` (than+đỏ) và `frame-pentagram-stat` (neon)
- Scene không số: `frame-build-minimal`, `frame-bold-poster`, `frame-creative-voltage`
- Scene list/comparison: `frame-aicoding-list`, `frame-aicoding-comparison`
...
- Dùng emoji inputs vừa phải (0–1 per field). ĐỪNG nhét emoji vào `hero` của `frame-build-minimal`.
...
- `headline` `frame-bold-poster`: ≤3 dòng, ≤14 ký tự/dòng.
