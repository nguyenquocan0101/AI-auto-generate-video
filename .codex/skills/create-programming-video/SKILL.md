...
```text
[Hook]                     ct-hook-hero
[Ví dụ đời thường]         ct-build-minimal   (bắt buộc cap2, tùy chọn cap3, bỏ nếu chung)
[Kiến thức nền]            ct-list            (bắt buộc cap2, tùy chọn cap3/chung)
[Giới thiệu]               ct-build-minimal
[Khái niệm]                ct-bold-poster
[Các bước]                 ct-list
[TRACE]                    ct-code-trace N
[Recap nhanh]              ct-list            (bắt buộc cap2/cap3, bỏ nếu chung)
[Độ phức tạp]              ct-stat-card
[Lỗi thường gặp]           ct-list            (nếu có)
[So sánh]                  ct-comparison      (nếu có optimization)
[Tối ưu hóa]               ct-bold-poster     (nếu có optimization)
...
Set `voice.speed` theo bảng v4. Sinh metadata top-level cho scene nếu có: `camera`, `pointer`,
`animations`, `transition`, `frames`, `changed_variables`.

## Self-check v4

- Scene đầu là `hook`, scene cuối là `outro`.
- Tổng scene và TRACE count khớp bảng v4.
- `cap2` có Ví dụ đời thường trước code/khái niệm.
- `cap2`/`cap3` có Recap nhanh ngay sau TRACE.
- `cap2`/`cap3` không nhảy cóc bước trung gian, trừ ngoại lệ graph/tree/vòng lặp dài.
- Scene so sánh/gán có nhịp dự đoán trước khi lộ đáp án.
- Thuật ngữ mới được định nghĩa ngay lần đầu.
- `voiceText` không chứa ký hiệu code; `inputs` được giữ code thật.
- `code_lines` trong các scene TRACE giống hệt nhau, chỉ đổi `active_line_num`.

## Yêu cầu hạ tầng

Renderer/schema trong repo phải hỗ trợ tối đa 24 scene, `metadata.audience`, và giữ các field
top-level như `camera`, `pointer`, `animations`, `transition`, `frames`, `changed_variables`.
